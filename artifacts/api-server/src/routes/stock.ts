...
router.post("/ingredients", requireAuth, async (req, res): Promise<void> => {
  ...
  if (!name || !unit || currentStock == null || minStock == null || !branchId) {
    res
      .status(400)
      .json({ error: "name, unit, currentStock, minStock, branchId required" });
    return;
  }
  const [item] = await db
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (costPerUnit !== undefined)
      updates.costPerUnit = costPerUnit?.toString();
...
router.get("/stock-movements", requireAuth, async (req, res): Promise<void> => {
  ...
  const ingredientId = req.query.ingredientId
    ? parseInt(req.query.ingredientId as string, 10)
    : undefined;

  const conditions = [];
  if (branchId) conditions.push(eq(stockMovementsTable.branchId, branchId));
  if (ingredientId)
    conditions.push(eq(stockMovementsTable.ingredientId, ingredientId));
...
router.post(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    if (!ingredientId || !type || quantity == null || !branchId) {
      res
        .status(400)
        .json({ error: "ingredientId, type, quantity, branchId required" });
      return;
    }
    const [m] = await db
...
router.get("/purchase-orders", requireAuth, async (req, res): Promise<void> => {
  ...
  const orders = branchId
    ? await db
        ...
        .where(eq(purchaseOrdersTable.branchId, branchId))
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const totalAmount = (items as Array<{ totalCost: number }>).reduce(
