...
function mapOrderItem(
  i: typeof orderItemsTable.$inferSelect & {
...
router.get("/orders", requireAuth, async (req, res): Promise<void> => {
  ...
  const tableId = req.query.tableId
    ? parseInt(req.query.tableId as string, 10)
    : undefined;
...
// Orders history - restricted to owner and manager
router.get(
  "/orders/history",
  requireAuth,
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const branchIdQuery = req.query.branchId
      ? parseInt(req.query.branchId as string, 10)
      : undefined;
    // If manager, restrict to their branch unless explicit branchId provided and role is owner
    const branchId =
      role === "owner" ? branchIdQuery : (req.userBranchId ?? undefined);
    ...
    let filtered = results.filter(Boolean) as any[];
    ...
    if (q) {
      ...
      filtered = filtered.filter((order) => {
        ...
        return order.items.some((i: any) =>
          i.menuItemName?.toLowerCase().includes(qLower),
        );
      });
    }
...
router.post(
  ...
  async (req: AuthRequest, res): Promise<void> => {
    ...
    const menuIds: number[] = items.map(
      (i: { menuItemId: number }) => i.menuItemId,
    );
...
router.patch(
  ...
  async (req, res): Promise<void> => {
    const itemId = parseInt(
      Array.isArray(req.params.itemId)
        ? req.params.itemId[0]
        : req.params.itemId,
      10,
    );
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [order] = await db
      .update(ordersTable)
      .set({ status: "voided", completedAt: new Date() })
      .where(eq(ordersTable.id, id))
      .returning();
    if (!order) {
      res.status(404).json({ error: "Order not found" });
